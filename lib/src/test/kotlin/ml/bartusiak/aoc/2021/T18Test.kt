...
class T18Test: StringSpec({
    ...
    "T181 should parse data" {
        (T181()).parseNumber("[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]") shouldBe
                Pair(SnailPair(left=SnailPair(left=SnailPair(left=Number(value=0), right=SnailPair(left=Number(value=5), right=Number(value=8))), right=SnailPair(left=SnailPair(left=Number(value=1), right=Number(value=7)), right=SnailPair(left=Number(value=9), right=Number(value=6)))), right=SnailPair(left=SnailPair(left=Number(value=4), right=SnailPair(left=Number(value=1), right=Number(value=2))), right=SnailPair(left=SnailPair(left=Number(value=1), right=Number(value=4)), right=Number(value=2)))), "")
    }

    "T181 should return correctly explode data" {
        T181().parseNumber("[[[[[9,8],1],2],3],4]").first.explode(0) shouldBe AddOp(left=9, right=0, newChild=T181().parseNumber("[[[[0,9],2],3],4]").first)
    }
    ...
    "T181 should return correctly explode data 3" {
        T181().parseNumber("[[[[0,7],4],[[7,8],[0,[6,7]]]],[1,1]]").first.explode(0) shouldBe AddOp(left=0, right=0, newChild=T181().parseNumber("[[[[0,7],4],[[7,8],[6,0]]],[8,1]]").first)
    }
