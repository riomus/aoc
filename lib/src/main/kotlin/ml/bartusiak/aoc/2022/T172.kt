...
open class T172 : AOCTask {
    ...
    open fun solve(file: String,stonesToWait : Long, start: Long, cycle: Long, height: Long, startHeight:Long, jetOffset: Long, stoneOffset: Long): Long{
        val jetPattern = data(file).first().split("").filter { it.isNotEmpty() }.map { rawJet ->
            jetMap[rawJet]!!
        }

        val gravity = StoneMove.Down(1)
        val stoneIndex = stoneOffset
        val jetIndex = jetOffset
        ...
        val toCycleStart =stonesToWait-start
        val toSimulate = toCycleStart%cycle
        val computed = (toCycleStart/cycle)*height
        val finalSim = generateSequence(simulation){(currentEnv, currentStone, currentStoneIndex, currentJetIndex) ->
            if(currentJetIndex%jetPattern.size == 0L && currentStoneIndex%stones.size==0L){
                println("$currentJetIndex,$currentStoneIndex,${currentEnv.top()}")
            }
        ...
        }.dropWhile { it.stoneIndex<=toSimulate+1 }.first()
        return finalSim.env.top()+computed+startHeight+1L
    }
