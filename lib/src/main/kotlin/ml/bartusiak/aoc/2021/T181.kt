            ...
            when(leftExplode){
                is NoOp -> {
                    ...
                    when(rightExplodeResult){
                        is NoOp -> NoOp
                        is AddOp -> AddOp(0, rightExplodeResult.right, SnailPair(left.addToRight(rightExplodeResult.left), rightExplodeResult.newChild))
                    }
                }
                is AddOp -> AddOp(leftExplode.left, 0, SnailPair(leftExplode.newChild, right.addToLeft(leftExplode.right)))
            }
        } else if (left is SnailPair && right is Number) {
            val explodeResult = left.explode(level+1)
            when(explodeResult){
                is NoOp -> NoOp
                is AddOp -> AddOp(explodeResult.left, 0, SnailPair(explodeResult.newChild, right.addToLeft(explodeResult.right)))
            }
