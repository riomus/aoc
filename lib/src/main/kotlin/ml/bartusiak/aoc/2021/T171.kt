...
open class T171 : AOCTask {
    ...
    data class SimulationStep(val initialState: ProbeState, val probeState: ProbeState, val maxY: Long, val isCorrect: Boolean,val shouldContinue: Boolean)
    ...
    fun simulationStep(step: SimulationStep, area: Area): SimulationStep{
        ...
        val newProbeState = ProbeState(state.x+state.dx, state.y+state.dy, if(state.dx==0L){0L} else if(state.dx<0){state.dx+1} else {state.dx-1}, state.dy-1)
        val isCorrect = newProbeState.x<=area.maxX && newProbeState.y>=area.minY
        val isInArea = newProbeState.x<=area.maxX && newProbeState.y>=area.minY && newProbeState.x>=area.minX && newProbeState.y<=area.maxY
   ...
   fun simulate(initD: Pair<Long, Long>, area: Area): SimulationStep {
       ...
       val result =
           generateSequence(SimulationStep(initState, initState, 0L, isCorrect = true, shouldContinue = true), step).dropWhile { it.shouldContinue }.first()
       return result
   }
