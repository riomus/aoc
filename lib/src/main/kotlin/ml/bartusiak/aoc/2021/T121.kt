...
open class T121: AOCTask{
    ...
    open fun solve(file: String, timesToVisitSingleSmall: Boolean): Int {
        ...
        val paths: List<List<String>> = findPaths(graph, "start", "end", graph["start"]!!.map { listOf("start", it) }, listOf(), timesToVisitSingleSmall)
        return paths.size
    }
    ...
    tailrec fun findPaths(graph: Map<String, Set<String>>, start: String, end: String, currentPaths: List<List<String>>,
                          finishedPaths: List<List<String>>,timesToVisitSingleSmall: Boolean): List<List<String>>{
        ...
        val continuedPaths: List<List<String>> = unfinishedPaths.flatMap { path ->
            ...
            val correctNextNodes = nextNodes.filter { node ->
                val lowerNodeRule = node.isLower() &&
                        ...
                        (!timesToVisitSingleSmall ||  (path+listOf(node)).filter { it.isLower() }.groupingBy { it }.eachCount().values.count { it>=maxVisitsForSmall }<2)
