...
open class T111 : AOCTask {
    ...
    val moves = listOf(Pair(1, 0), Pair(0, 1), Pair(1, 1), Pair(-1, 1)).flatMap {
        listOf(
            it,
    ...
    fun nextSquids(state: Pair<Map<Int, Map<Int, Int>>, Int>): Pair<Map<Int, Map<Int, Int>>, Int> {
        ...
        val startZeros =
            newMap.entries.flatMap { (x, row) -> row.entries.filter { (_, v) -> v == 0 }.map { (y, _) -> Pair(x, y) } }
    ...
    tailrec fun step2(map: Map<Int, Map<Int, Int>>, zeros: List<Pair<Int, Int>>): Map<Int, Map<Int, Int>> {
        return if (zeros.isEmpty()) {
            ...
            val newMap: Map<Int, Map<Int, Int>> =
                increments.entries.filter { entry -> map[entry.key.first]!![entry.key.second]!! > 0 }
                    .fold(map) { lastMap, entry ->
                        val (x, y) = entry.key
                        val inc = entry.value
                        val incrementedValue = lastMap[x]!![y]!! + inc
                        val newMap =
                            lastMap + Pair(x, lastMap[x]!! + Pair(y, if (incrementedValue > 9) 0 else incrementedValue))
                        newMap
                    }
            val newZeros: List<Pair<Int, Int>> = newMap.entries.flatMap { (x, row) ->
                row.entries.flatMap { (y, value) ->
