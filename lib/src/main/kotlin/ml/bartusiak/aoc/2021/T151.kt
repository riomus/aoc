...
open class T151 : AOCTask {
    ...
    val moves: Sequence<Pair<Int, Int>> = listOf(Pair(1,0), Pair(0,1)).flatMap { listOf(it, Pair(it.first*-1, it.second*-1)) }.asSequence()
    companion object {
        fun solve(file: String = "T15.txt"): Long = T151().solve(file)
    }

    data class DjikstraState(val unvisitedQueue: PriorityQueue<Pair<Pair<Int, Int>, Long>>, val unvisitedNodes: MutableSet<Pair<Int, Int>>,
    ...
    fun djikstra(startNode: Pair<Int, Int> , endNode: Pair<Int,Int>, map: Map<Pair<Int, Int>, Int>):Long{
        ...
        val solution = generateSequence(DjikstraState(unvisitedQueue, map.keys.toMutableSet(), startDistances.toMutableMap(), initialNode), thisDjikstraStep).dropWhile { state -> state.unvisitedNodes.contains(endNode) }.first()
    ...
    fun djijkstraStep(map:  Map<Pair<Int, Int>, Int>, state: DjikstraState): DjikstraState {
        ...
        return if(currentNode!=null){
            val unvisitedNeighbours = moves.map { it+currentNode }.filter { distances.contains(it) }.filter{unvisitedSet.contains(it)}
